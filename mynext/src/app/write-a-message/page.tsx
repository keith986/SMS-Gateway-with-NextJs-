...
import handleSubmitMessage from "../infobip-actions.ts"

export default function WriteAMessagePage() {
   const { sessionId } = useAuth()
   const [allClients, setAllClients] = useState([])
   const [mySearch, setMySearch] = useState([])
   const [myAddedClient, setMyAddedClient] = useState([])

   const docRef = collection(db, "clientusers")

   onSnapshot(docRef, (snapshot) => {
        let client_data = [];
        snapshot.docs.forEach((doc) => {
          client_data.push({...doc.data(), id : doc.id})
        })
        setAllClients(client_data)
   }) 
   ...
   const handleAddClient = (e) => {
     allClients.map((clnt) => {
      if(clnt.id === e.target.id){
         myAddedClient.push(clnt)
      }
     })
     setMyAddedClient(myAddedClient)
   }

   const handleRemoveClient = (eve) => {  
      const remove = myAddedClient.filter((c) => c.id !== eve.target.id)
      setMyAddedClient(remove)
   }

   const handleSearchFilter = (ev) => {
      const mysearch = allClients.filter((s) => s.username.includes(ev.target.value))
...
return (
    <>
<aside id="separator-sidebar" className="fixed top-20.5 left-0 z-40 w-64 h-screen transition-transform -translate-x-full sm:translate-x-0" aria-label="Sidebar">
   <div className="h-full px-3 py-4 overflow-y-auto bg-gray-50 dark:bg-gray-800">
      <ul className="space-y-2 font-medium">
         ...
         <li>
            <a href="/write-a-message" className="flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 group">
         ...
         <li>
            <a href="/inbox" className="flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 group">
         ...
         <li>
            <SignOutButton signOutOptions={{ sessionId }} className="w-full flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-red-100 dark:hover:bg-red-700 group cursor-pointer" />
         </li>
      </ul>
      <ul className="pt-5 mt-5 space-y-2 font-medium border-t border-gray-200 dark:border-gray-700">
         <li>
            <a href="/subscription-and-billing" className="flex items-center p-2 text-gray-900 transition duration-75 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 dark:text-white group">
   ...
   <div className="p-4 border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700">
      <div className="grid grid-cols-2 gap-4 mb-2">
         ...
         <div className="flex items-center justify-center rounded-sm bg-gray-50 dark:bg-gray-800">
               <button id="dropdownDefaultButton" data-dropdown-toggle="dropdown" className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 cursor-pointer" type="button" onClick={handleDropDown}>SELECT<svg className="float-right w-4 h-2.5 ms-3 cursor-pointer" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 10 6" >
      ...
      <div id="dropdowns" className="overflow-y-auto z-10 hidden bg-white divide-y divide-gray-100 rounded-lg shadow-sm dark:bg-gray-700 mb-4 max-h-50">
               <ul className="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownDefaultButton">
                  <li>
                     <a href="#" className="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white" onClick={handleToggleClients}>All user</a>
                  </li>
                  <li>
                     <Input name="search" className="m-2 block pt-2 ps-10 text-sm text-gray-900 border border-gray-300 rounded-lg w-80 bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Custom user(s): Search by username" onChange={handleSearchFilter}/>
                  </li>
               </ul>
         {mySearch.map((clent) => {
            return (
            <Button className="mx-1 cursor-pointer"  id={clent.id} onClick={handleAddClient}>
            {clent.username} {clent.roomno} {clent.location}
            <svg className="w-8 h-8 text-gray-800 dark:text-white cursor-pointer" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
      ...
      <div className="overflow-y-auto z-10 bg-white divide-y divide-gray-100 rounded-lg shadow-sm dark:bg-white-700 mb-4 max-h-50 p-2">
         {myAddedClient.map((client) => {
            return (
            <Button className="mx-1 cursor-pointer" id={client.id} onClick={handleRemoveClient}>
            {client.username} {client.roomno} {client.location}
            <svg className="w-8 h-8 text-red-800 dark:text-red cursor-pointer" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
...
<div className="w-full relative overflow-x-auto p-2 sm:rounded-lg">
 <form action={handleSubmitMessage}>
   <input type="hidden" id="rname" name="rname" value={myAddedClient.map(c => c.roomno)}/>
   <input type="hidden" id="receipts" name="receipts" value={myAddedClient.map(c => c.phonenumber)}/>
  <textarea name="message" className="w-full bg-gray-50 min-h-50 p-2" placeholder="Compose a new message" id="message"></textarea>
  <Button type="submit" variant="secondary" className="float-right cursor-pointer mt-5">Send a message</Button>
 </form>
 </div>
</div>
